...
TEST_F(SchemaValidatorTest, FactoryGetRuntimeJavaHistoryValidator)
{
    SchemaValidatorFactory& factory = SchemaValidatorFactory::getInstance();
    factory.reset();
    factory.initialize();

    auto validator = factory.getValidator("wazuh-states-vulnerabilities-runtime-java-history");
    if (!validator)
    {
        GTEST_SKIP() << "Runtime Java vulnerability history schema validator not available";
    }

    EXPECT_EQ(validator->getSchemaName(), "wazuh-states-vulnerabilities-runtime-java-history");
}

TEST_F(SchemaValidatorTest, RuntimeJavaHistoryValidatorAcceptsTransitionDocumentShape)
{
    SchemaValidatorFactory& factory = SchemaValidatorFactory::getInstance();
    factory.reset();
    factory.initialize();
    ...
    nlohmann::json message = {
        {"schema_version", "1.0"},
        {"event", {{"kind", "state"},
                    {"category", nlohmann::json::array({"vulnerability"})},
                    {"type", nlohmann::json::array({"change"})},
                    {"action", "resolved"},
                    {"outcome", "success"},
        ...
        {"wazuh", {{"agent", {{"id", "001"},
                                 ...
                                 {"host", {{"hostname", "demo-host"},
                                            ...
                                            {"os", {{"name", "Ubuntu"},
                                                     ...
                                                     {"version", "22.04"}}}}}}},
                    {"cluster", {{"name", "wazuh"}, {"node", "node01"}}},
                    {"session", {{"id", "43"}, {"module", "syscollector_vd_runtime_java_full"}, {"scan_mode", "full"}}}}},
        {"matcher", {{"source", "runtime-java-matcher"},
                      {"url", "http://127.0.0.1:8080/runtime-java/match"},
                      {"request_id", "42"},
                      {"generated_at", "2026-05-08T09:00:00.000Z"}}},
        {"runtime_java",
         {{"inventory_id", "bc20a4ff0d0dc25a8c6ea42f77bc5c0e8cc6e9a1"},
          ...
          {"history", {{"action", "resolved"},
                        {"current_document_id", "runtime-java:abc123"},
                        {"transitioned_at", "2026-05-08T10:00:00.000Z"}}},
          {"component", {{"_inventory_id", "bc20a4ff0d0dc25a8c6ea42f77bc5c0e8cc6e9a1"},
                         {"_inventory_index", "wazuh-states-inventory-runtime-java-components"},
                         {"_document_version", 7},
                         {"package_type", "jar"},
